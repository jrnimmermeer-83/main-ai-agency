...
export default function Workspace({ section }: { section: Section }) {
  ...
  const retryQueries = () => {
    void Promise.all([organizations.refetch(), active.refetch(), overview.refetch(), changes.refetch(), history.refetch(), providers.refetch()]);
  };
  const forceTenantErrorForLocalVerification = import.meta.env.DEV && new URLSearchParams(window.location.search).has("forceTenantError");
  const forceTenantLoadingForLocalVerification = import.meta.env.DEV && new URLSearchParams(window.location.search).has("forceTenantLoading");
  const hasQueryError = forceTenantErrorForLocalVerification || organizations.isError || active.isError || (hasContext && (overview.isError || changes.isError || history.isError || providers.isError));

  if (forceTenantLoadingForLocalVerification || organizations.isLoading || active.isLoading) return <DashboardLayout><div className="grid min-h-[50vh] place-items-center text-slate-500" role="status" aria-live="polite"><Loader2 className="size-6 animate-spin" /><span className="sr-only">Loading tenant workspace</span></div></DashboardLayout>;
  if (hasQueryError) return <DashboardLayout><section className="mx-auto mt-12 max-w-xl rounded-3xl border border-amber-200 bg-white p-8 shadow-xl shadow-slate-200/50"><div className="grid size-12 place-items-center rounded-2xl bg-amber-100 text-amber-700"><AlertTriangle className="size-6" /></div><p className="mt-7 text-sm font-semibold text-amber-700">Workspace unavailable</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">We could not load this tenant safely.</h1><p className="mt-3 leading-6 text-slate-600">No configuration change was made. Check your connection or retry to reload the tenant-scoped data.</p><Button onClick={retryQueries} className="mt-7 bg-slate-950 text-white hover:bg-slate-800"><RotateCcw className="size-4" /> Retry secure load</Button></section></DashboardLayout>;
